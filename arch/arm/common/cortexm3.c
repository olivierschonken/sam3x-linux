...
void cortex_m3_reboot(void)
{
    ...
    defined(CONFIG_ARCH_LPC178X) || defined(CONFIG_ARCH_LPC18XX) || defined(CONFIG_ARCH_AT91SAM3X8H)
/*
 ...
 */
static u32 notrace cortexm3_read_sched_clock(void)
{
	return clocksource_systick_value_get(&clocksource_systick);
}

int __devinit read_current_timer(unsigned long *timer_value)
{
	*timer_value = clocksource_systick_value_get(&clocksource_systick);
	return 0;
}
/*
...
void cortex_m3_register_systick_clocksource(u32 systick_clk)
{
	 ...
	 */
	clocks_calc_mult_shift(&clocksource_systick.mult, &clocksource_systick.shift, systick_clk,
					      NSEC_PER_SEC, 1);
	setup_sched_clock(cortexm3_read_sched_clock, 24,
			systick_clk);
	clocksource_register_hz(&clocksource_systick, systick_clk);
}
